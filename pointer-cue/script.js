...
function selectOS(os, remember = true) {
  ...
  if (remember) {
    try { localStorage.setItem('pointerCueSelectedOS', os); } catch (_) {}
  }
...
function detectOS() {
  ...
  try {
    const saved = localStorage.getItem('pointerCueSelectedOS');
...
if (compareSection) {
  ...
  comparisonObserver.observe(compareSection);
}
